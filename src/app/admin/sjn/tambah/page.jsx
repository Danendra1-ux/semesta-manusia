...
export default function TambahSJNProgramPage() {
  ...
  // Formulir creation tracking — hanya aktif setelah admin buat formulir
  const [formulirCreatedFF, setFormulirCreatedFF] = useState(false);
  const [formulirCreatedSF, setFormulirCreatedSF] = useState(false);
  ...
  // Tooltip state for "Buat Formulir" buttons
  const [tooltipFf, setTooltipFf] = useState(false);
  const [tooltipSf, setTooltipSf] = useState(false);

  // Locked state for Batas Registrasi & Status — hanya unlock jika formulir sudah dibuat
  const isLockedFF = !formulirCreatedFF;
  const isLockedSF = !formulirCreatedSF;

  // 1. Load draft from sessionStorage on mount
  useEffect(() => {
    if (typeof window !== "undefined") {
      const ffCreated = sessionStorage.getItem("sjn_formulir_created_ff");
      const sfCreated = sessionStorage.getItem("sjn_formulir_created_sf");
      if (ffCreated === "true") setFormulirCreatedFF(true);
      if (sfCreated === "true") setFormulirCreatedSF(true);
      ...
      if (draft) {
        ...
        if (parsed.fullyFundedStatus) setFullyFundedStatus(parsed.fullyFundedStatus);
        if (parsed.selfFundedStatus) setSelfFundedStatus(parsed.selfFundedStatus);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      sessionStorage.setItem("sjn_draft", JSON.stringify({
        ...
        fullyFundedBatasReg, selfFundedBatasReg,
        fullyFundedStatus, selfFundedStatus,
        detailFields, pekerjaanFields
      }));
    }
  }, [nama, jadwalMulai, jadwalSelesai, lokasi, deskripsi, fullyFundedBatasReg, selfFundedBatasReg, fullyFundedStatus, selfFundedStatus, detailFields, pekerjaanFields]);

  // Check if navigated back from formulir page with created=true
  useEffect(() => {
    const created = searchParams.get("created");
    const tipe = searchParams.get("tipe");
    if (created === "true" && tipe) {
      if (tipe === "fully-funded") {
        setFormulirCreatedFF(true);
        if (typeof window !== "undefined") {
          sessionStorage.setItem("sjn_formulir_created_ff", "true");
        }
      } else if (tipe === "self-funded") {
        setFormulirCreatedSF(true);
        if (typeof window !== "undefined") {
          sessionStorage.setItem("sjn_formulir_created_sf", "true");
        }
      }
  ...
  const handleBuatFormulir = (tipe) => {
    if (!nama.trim()) {
      if (tipe === "ff") {
        setTooltipFf(true);
        setTimeout(() => setTooltipFf(false), 3000);
      } else {
        setTooltipSf(true);
        setTimeout(() => setTooltipSf(false), 3000);
      }
      return;
    }
    // Navigate to formulir page — use a placeholder id that the formulir page handles
    router.push(`/admin/sjn/tambah/formulir?tipe=${tipe}&nama=${encodeURIComponent(nama)}`);
  };
  ...
  const handleSave = () => {
    ...
    if (fullyFundedBatasReg && jadwalMulai && new Date(fullyFundedBatasReg) > new Date(jadwalMulai)) {
      return showToast("Batas Registrasi Fully Funded tidak boleh setelah jadwal mulai!", true);
    }
    if (selfFundedBatasReg && jadwalMulai && new Date(selfFundedBatasReg) > new Date(jadwalMulai)) {
    ...
    sessionStorage.removeItem("sjn_poster_name");
    sessionStorage.removeItem("sjn_poster_type");
    sessionStorage.removeItem("sjn_formulir_created_ff");
    sessionStorage.removeItem("sjn_formulir_created_sf");

    // Redirect to list page
    router.push("/admin/sjn");
  };
                ...
                <div className={styles.tipeRow}>
                  <div className={styles.tipeFieldGroup}>
                    ...
                    <div className={styles.inputWrapper}>
                      <input
                        type="date"
                        className={`${styles.input} ${isLockedFF ? styles.inputLocked : ""}`}
                        value={fullyFundedBatasReg}
                        onChange={(e) => setFullyFundedBatasReg(e.target.value)}
                        disabled={isLockedFF}
                        placeholder={isLockedFF ? "Belum Dibuat" : ""}
                      />
                      {isLockedFF ? (
                        <svg className={`${styles.inputIcon} ${styles.lockIcon}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <div className={styles.tipeBtnWrapper}>
                    <button
                      className={`${styles.viewFormBtn} ${formulirCreatedFF ? styles.viewFormBtnCreated : ""}`}
                      onClick={() => handleBuatFormulir("ff")}
                      onMouseEnter={() => !nama.trim() && setTooltipFf(true)}
                      onMouseLeave={() => setTooltipFf(false)}
                    >
                      {formulirCreatedFF ? (
                        <>
                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <div className={styles.tipeRow}>
                  <div className={styles.tipeFieldGroup}>
                    ...
                    <div className={styles.inputWrapper}>
                      <input
                        type="date"
                        className={`${styles.input} ${isLockedSF ? styles.inputLocked : ""}`}
                        value={selfFundedBatasReg}
                        onChange={(e) => setSelfFundedBatasReg(e.target.value)}
                        disabled={isLockedSF}
                        placeholder={isLockedSF ? "Belum Dibuat" : ""}
                      />
                      {isLockedSF ? (
                        <svg className={`${styles.inputIcon} ${styles.lockIcon}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                  ...
                  <div className={styles.tipeBtnWrapper}>
                    <button
                      className={`${styles.viewFormBtn} ${formulirCreatedSF ? styles.viewFormBtnCreated : ""}`}
                      onClick={() => handleBuatFormulir("sf")}
                      onMouseEnter={() => !nama.trim() && setTooltipSf(true)}
                      onMouseLeave={() => setTooltipSf(false)}
                    >
                      {formulirCreatedSF ? (
                        <>
                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
