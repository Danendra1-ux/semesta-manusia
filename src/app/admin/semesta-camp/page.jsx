...
export default function SemestaCampPage() {
  ...
  const fetchPrograms = async () => {
    try {
      const response = await fetch('/api/programs?category=Semesta Camp', { cache: 'no-store' });
  ...
  const openDeleteModal = (id) => {
    ...
    setDeleteModal({
      open: true,
      id,
      title: target?.title || "Program ini",
      pendaftar: target?.registration_count ?? 0,
    });
    setActiveDropdown(null);
  };
        ...
        >
          <button
            ...
            onClick={() => {
              ...
              setActiveDropdown(null);
              router.push(`/admin/semesta-camp/${program.id}/edit`);
            }}
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <button
            ...
            onClick={() => {
              const program = paginatedPrograms.find((p) => p.id === activeDropdown);
              handleToggleStatus(program.id, program.status || "Dibuka");
            }}
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
